...
class RgNameSurname: UIViewController {
    ...
    lazy var nameSurnameText2:UITextView = {
        ...
        infoText.textColor = .black
        infoText.textAlignment = .center
        infoText.font = UIFont(name: "verdana", size: 10)
        infoText.isEditable = false
        return infoText
    }()
    ...
    @objc func nextButtonTapped(click : UIButton!){
        RegistrationInformation.rgİnformation.name = name.text
        RegistrationInformation.rgİnformation.surname = surname.text
        
        navigationController?.pushViewController(rgPhone(), animated: true)
